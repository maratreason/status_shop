...
/* FORMS */
var modalSuccess = new bootstrap.Modal(document.getElementById('success-message'), {});
var modalCallback = new bootstrap.Modal(document.getElementById('callback'), {});
var modalQuiz_close = new bootstrap.Modal(document.getElementById('quiz'), {});
...
window.addEventListener("load", function () {
  function sendData() {
    ...
    XHR.addEventListener("error", function (event) {
      alert('Извините, при отправке формы возникла ошибка. Повторите пожалуйста.');
    });
    XHR.open("POST", "https://salon-status.online/mail/send2.php");
    XHR.send(FD);
  }
  const form = document.getElementById("form_recording");
...
window.addEventListener("load", function () {
  function sendData() {
    ...
    XHR.addEventListener("load", function (event) {
      modalSuccess.show();
      modalQuiz_close.hide();
      document.getElementById("form_color").reset();
      clearQuiz('form_color');
    });
    XHR.addEventListener("error", function (event) {
      alert('Извините, при отправке формы возникла ошибка. Повторите пожалуйста.');
    });
    XHR.open("POST", "https://salon-status.online/mail/send.php");
    XHR.send(FD);
  }
